...
def _call_ollama_raw(prompt: str, *, max_tokens: int = 4000, temperature: Optional[float] = None) -> Optional[str]:
...
def call_glm_api(prompt: str, *, max_tokens: int = 4000, temperature: Optional[float] = None) -> Optional[str]:
    """统一 LLM 调用入口，根据 LLM_BACKEND 路由"""
    if LLM_BACKEND == "ollama":
        return _call_ollama_raw(prompt, max_tokens=max_tokens, temperature=temperature)
    return _call_glm_raw(prompt, max_tokens=max_tokens, temperature=temperature)
...
def estimate_emotion_axes(content: str) -> Dict[str, float]:
    lexicon = {
        "positive": ["笑", "喜", "轻松", "温暖", "安心", "满足", "希望"],
        "negative": ["痛", "恨", "怕", "绝望", "愤怒", "羞辱", "崩溃"],
        "tension": ["杀", "对决", "追击", "危机", "硬撑", "压迫", "危险", "反击"],
        "suspense": ["秘密", "真相", "谜", "阴谋", "不对劲", "线索", "诡异"],
        "release": ["回报", "揭晓", "成功", "击败", "拥抱", "释然", "兑现"],
    }
...
def extract_potential_characters(content: str, limit: int = 30) -> List[str]:
    pattern = re.compile(r"([\u4e00-\u9fa5]{2,4})(?:说|道|问|笑|看着|想着|点头|摇头)")
    names = [match.group(1) for match in pattern.finditer(content)]
    filtered = [name for name in names if name not in {"自己", "众人", "对方", "其中", "如果"}]
...
def build_character_cooccurrence(chapters: List[ChapterRecord]) -> Dict[str, Dict[str, int]]:
    ...
    for chapter in chapters:
        characters = extract_potential_characters(chapter.content, limit=12)
...
def _tier1_analyze_from_chapters(chapters: List[ChapterRecord], book_name: str) -> Tuple[Dict[str, Any], List[ChapterRecord], List[Dict[str, Any]]]:
    ...
    high_value = scored_stats[: min(TIER2_MAX_CHAPTERS, len(scored_stats))]
    character_map = build_character_cooccurrence(chapters)
    plot_keywords = extract_plot_keywords(chapters)
    potential_characters = extract_potential_characters("\n".join(ch.content for ch in chapters[:50])[:120000])
...
def build_tier2_prompt(book_name: str, chapter_num: int, chapter_title: str, content: str, chapter_stats: Dict[str, Any]) -> str:
    return f"""
你是一位严格执行知识底座规范的网文蒸馏器。

目标：从单章内容中提取 3-5 条"候选知识条目"，要求它们以后能直接进入审核/批准/回流流程。
...
- emotion_axes: {json.dumps(chapter_stats.get('emotion_axes', {}), ensure_ascii=False)}
- key_dialogues: {json.dumps(chapter_stats.get('key_dialogues', []), ensure_ascii=False)}
...
{{
  "items": [
    {{
      ...
      "principle": "一句话为什么有效，30-50字",
      "mechanism": "一句话核心做法，30-50字",
      "application": "适用场景关键词，10-20字",
      "evidence": ["原文片段"],
      "confidence": 0.0,
      "reader_effect_tags": ["代入", "揪心"],
      "quality_axes": ["沉浸", "感染"],
      "targets": ["人物", "关系"],
      "stages": ["中段", "卷末"],
      "control_tags": ["需人工确认", "高频"],
      "risk_tags": ["易模板化"],
      "usable_modules": ["启动包", "质量引擎"],
      "usable_checkpoints": [2, 3, 4, 5]
    }}
  ]
}}
...
1. 不要输出读后感，不要空泛夸奖。
2. 每条必须可复用，且能说明为什么有效。
3. principle 必须写满 30-50 字，详细解释"为什么有效"；mechanism 必须写满 30-50 字，具体描述"核心做法"；application 必须写满 10-20 字。宁可多写也不要太短。
4. knowledge_type / level 必须使用英文枚举值。
5. quality_axes / targets / stages / reader_effect_tags / control_tags / risk_tags 必须是数组。
6. confidence 必须是 0-1 之间的小数。
7. 如果证据不足，就不要硬造条目。
"""


def build_tier3_prompt(book_name: str, warehouse: str, items: List[Dict[str, Any]]) -> str:
    # 最多送 32 条给 LLM 归纳，平衡多样性与 token 预算
...
"""
    ...
    return f"""
你是一位严格执行知识底座规范的深度提炼器。

目标：把同一仓库下的 Tier 2 候选知识进行去重、归纳、升格，生成 3-6 条真正可审批的 Tier 3 候选知识。
...
{{
  "items": [
    {{
      ...
      "knowledge_type": "template/rule/pattern/archetype/graph/ledger/sample/counter_example/trigger/signal/arc",
      "level": "book/volume/story_segment/chapter/scene/beat/paragraph/sentence",
      "principle": "核心原理，50-80字",
      "mechanism": "核心做法，50-80字",
      "application": "适用场景，20-40字",
      "evidence": ["原文片段，最多2条"],
      "confidence": 0.0,
      "reader_effect_tags": ["代入", "讨论"],
      "quality_axes": ["牵引", "沉浸"],
      "targets": ["结构", "情绪"],
      "stages": ["开篇", "卷末"],
      "control_tags": ["需人工确认", "高频"],
      ...
      "usable_checkpoints": [5, 6, 7]
    }}
  ]
}}

严格要求：
1. 只输出可进入 candidate review 的条目，不要输出泛泛总结。
2. 精简输出：principle/mechanism/application 要简洁有力，去掉冗余。
3. 优先保留可直接改善启动包、质量裁决、长篇稳态生产的知识。
4. 如果多个来源条目只是换说法，要合并成一条更强的 principle，而不是平铺重复。
"""
    ...
    *,
    ...
    if TIER2_CONCURRENCY <= 1:
        ...
        for index, candidate in enumerate(high_value, start=1):
            ...
            content = clip_text(chapter.content, 3000)
